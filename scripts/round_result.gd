...
func _build_victory() -> void:
	...
	_af(title, 112); add_child(title)

	var blind_lbl = Label.new()
	blind_lbl.text = _t(blind_name)
	blind_lbl.position = Vector2(0, 550)
	...
	_af(blind_lbl, 48); add_child(blind_lbl)

	var score_lbl = Label.new()
	score_lbl.text = str(score) + " / " + str(target)
	score_lbl.position = Vector2(0, 680)
	...
	_af(score_lbl, 80); add_child(score_lbl)

	var income_title = Label.new()
	income_title.text = _t("EARNINGS")
	income_title.position = Vector2(0, 860)
	...
	_af(income_title, 44); add_child(income_title)

	var income_lbl = Label.new()
	income_lbl.text = "+ $" + str(income)
	income_lbl.position = Vector2(0, 930)
	...
	_af(income_lbl, 96); add_child(income_lbl)

	var shop_btn = Button.new()
	shop_btn.text = _t("Go to Shop")
	shop_btn.custom_minimum_size = Vector2(440, 100)
	shop_btn.position = Vector2(CENTER_X - 220, 1160)
...
func _build_defeat() -> void:
	...
	var blind_lbl = Label.new()
	blind_lbl.text = _t(blind_name)
	blind_lbl.position = Vector2(0, 650)
	blind_lbl.custom_minimum_size = Vector2(SCREEN_W, 0)
	blind_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	blind_lbl.add_theme_color_override("font_color", Color(0.7, 0.7, 0.65))
	_af(blind_lbl, 48); add_child(blind_lbl)
	...
	var score_lbl = Label.new()
	score_lbl.text = str(score) + " / " + str(target)
	score_lbl.position = Vector2(0, 780)
	score_lbl.custom_minimum_size = Vector2(SCREEN_W, 0)
	score_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	...
	restart_btn.custom_minimum_size = Vector2(440, 100)
	restart_btn.position = Vector2(CENTER_X - 220, 1000)
